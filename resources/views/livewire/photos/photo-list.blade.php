                    ...
                    <div x-data="{ 
                            ...
                            @foreach($photos as $photo)
                                                    <div class="album-group mb-8">
                                                        ...
                                                        <div class="flex items-center justify-between border-b border-gray-100 pb-2 mb-3 px-1">
                                                            <div class="flex items-baseline gap-2 relative"
                                                                wire:key="album-header-{{ $photo->id }}">
                                                            ...
                                                            <div class="flex items-center gap-2">
                                                                ...
                                                                <!-- resources/views/livewire/album-type-badge.blade.php -->

                                                            <div class="relative inline-block" x-data="{ open: false }">
                                                                <!-- Display Badge (Click to Edit) -->
                                                                <button 
                                                                    @click="open = !open"
                                                                    class="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-blue-50 border border-blue-100 text-[10px] font-bold text-blue-600 uppercase tracking-tight shadow-sm hover:bg-blue-100 transition-colors"
                                                                >
                                                                    <span>{{ $options[$photo->priority] }}</span>
                                                                </button>
                                                                ...
                                                                <div 
                                                                    ...
                                                                    x-transition
                                                                    class="absolute z-10 mt-2 w-32 origin-top-left rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                                                                >
                                                                    <div class="py-1">
                                                                        @foreach($options as $opt=>$option)
                                                                            <button 
                                                                                wire:click="updateType('{{ $opt }}', {{ $photo->uuid }})"
                                                                                @click="open = false"
                                                                                class="block w-full text-left px-4 py-2 text-xs font-medium text-gray-700 hover:bg-gray-100 {{ $photo->priority === $opt ? 'bg-blue-50 text-blue-700' : '' }}"
                                                                            >
                                                                                {{ $option }}
                                                                            </button>
                                                                        @endforeach
                                ...
                                " class="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-100 text-[10px] font-bold text-emerald-600 uppercase tracking-tight shadow-sm hover:bg-emerald-100 transition-colors">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                                                                        fill="currentColor" class="size-3">
                                                                        <path
                                                                            d="M15 11a3 3 0 1 1-2.83 2H5.83a3 3 0 1 1 0-2h6.34A3 3 0 0 1 15 11Z" />
                                                                    </svg>
                                                        ...
                                                        <div class="flex flex-wrap gap-1.5">
                                                            ...
                                                            @foreach($photo->attachments as $index => $attachment)
                                                                <div wire:key="attachment-{{ $attachment->id }}"
                                                                    ...
                                                                    <div @click="openModal({{ $jsAttachments->toJson() }}, {{ $index }})"
                        ...
                        <div x-show="open" x-transition.opacity @keydown.escape.window="open = false"
                            ...
                            <button @click="open = false"
                                class="absolute top-5 right-5 text-white/70 hover:text-white z-[60]">
                            ...
                            <div class="relative w-full h-full flex items-center justify-center">
                                <template x-if="attachments.length > 1">
                                    <div class="absolute inset-x-0 flex justify-between px-4 z-50">
                                        <button @click="prev()"
                                            class="p-3 rounded-full bg-white/10 text-white hover:bg-white/20 transition">
